...
class SequenceModel():
    ...
    def loss_fn(self, pred, label, weights):
        ...
        pred = pred[mask]
        label = label[mask]
        mse_loss = torch.mean((pred - label) ** 2)
    ...
    def predict(self, dl_test, batch_size = 32):
        ...
        for batch_data in test_loader:
            ...
            # 计算这个批次的weighted_r2 - 评估仍然使用weighted_r2
...
def main(args):
    ...
    data_path = os.path.join(base_dir, f'../data/output/Fature_factor_data_{day}_master.parquet')
...
if __name__ == "__main__":
    ...
    base_dir = os.path.dirname(os.path.abspath(__file__))
    default_save_path = os.path.join(base_dir, 'models/MASTER_mse')
    # 路径参数
