...
class SequenceModel():
    ...
    def fit(self, dl_train, dl_valid, patience=5, min_delta=0.001, batch_size=32):
        ...
        #model_path = os.path.join(absolute_save_path, f"{self.save_prefix}_beta_{self.beta}_master.pkl")
...
def main(args):
    ...
    data_path = os.path.join(base_dir, f'../data/output/Processed_data_{day}.parquet')
    ...
    print(f"Model Trained with d_model={args.d_model}, dropout={args.dropout}")
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--gate_input_end_index', type=int, default=89,
    ...
    base_dir = os.path.dirname(os.path.abspath(__file__))
    default_save_path = os.path.join(base_dir, 'models/MASTERnogate30')
    # 路径参数
    ...
    #                     help='模型文件名前缀')
    # 修改参数默认值，去掉前导斜杠
    parser.add_argument('--save_prefix', type=str, default='master0424',
                        help='模型文件名前缀')
